...
(function($) {
	...
	function handleSetThemeOption(item, index, arr) {
		...
		if(attrName === "sidebarbg" || attrName === "primary" || attrName === "headerbg" || attrName === "nav_headerbg" ){
			...
			var element = document.getElementById(attrNameColor+"_"+item.value);
			if(element) {
				element.checked = true;
			}
		}else if(attrName === "navigationbarimg"){
			var imgElement = document.getElementById("sidebar_img_"+item.value.split('sidebar-img/')[1].split('.')[0]);
			if(imgElement) {
				imgElement.checked = true;
			}
		}else if(attrName === "sidebartext"){
			var textElement = document.getElementById("sidebar_text_"+item.value);
			if(textElement) {
				textElement.checked = true;
			}
		}else if(attrName === "direction" || attrName === "nav_headerbg" || attrName === "headerbg"){
			var dirElement = document.getElementById("theme_direction");
			if(dirElement) {
				dirElement.value = item.value;
			}
			...
			var styleElement = document.getElementById(attrName);
			if(styleElement) {
				styleElement.value = item.value;
			}
		}else if(attrName === "layout"){
			if(item.value === "vertical"){return false}
			var layoutElement = document.getElementById("theme_layout");
			if(layoutElement) {
				layoutElement.value = item.value;
			}
		}
		else if(attrName === "container"){
			if(item.value === "wide"){return false}
			var containerElement = document.getElementById("container_layout");
			if(containerElement) {
				containerElement.value = item.value;
			}
		}
