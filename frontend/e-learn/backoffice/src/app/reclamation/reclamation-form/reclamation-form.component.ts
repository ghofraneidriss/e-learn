...
export class ReclamationFormComponent implements OnInit {
  ...
  exportPdf(item: Reclamation): void {
    ...
    exporter.setFontSize(16);
    exporter.text('Reclamation', 14, 18);
    ...
    for (const line of lines) {
      const wrapped = exporter.splitTextToSize(line, 80);
      exporter.text(wrapped, 14, y);
      y += wrapped.length * 7;
    }

    exporter.save(`reclamation-${item.id ?? 'item'}.pdf`);
  }
